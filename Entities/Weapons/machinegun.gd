...
var wpnname = "machinegun"
const ammoMax = 250
const ammoFit = 4 # 50
var ammoOut = 8 # onready & current
var ammoIn = 4  # onready & current #25
...
func current():
	...
	if player.select == inventory:
		player.SpriteArm.texture = player.sashot
		...
		if busy == 0:
			if ammoIn > 0:
				if Input.is_action_pressed("ply_attack2"):
