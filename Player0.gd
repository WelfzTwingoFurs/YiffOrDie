...
func _process(_delta): ############################################################# SPRITE WORK ###
	#$Sprite.frame = frame + ((spr_step*int(gun)) * (0 if ((pistol[frame].x == 0) && (pistol[frame].y == 0)) else 1))
	...
	match gun:
		false: $Arm.texture = spr_emptyhanded
		true: $Arm.texture = spr_onehanded
	...
	if holding:
		if (pistol[frame].x == 0) && (pistol[frame].y == 0):
...
@export var spr_emptyhanded = preload("res://Assets/PlayerWeggy/weg_emptyhanded.png")
@export var spr_onehanded = preload("res://Assets/PlayerWeggy/weg_onehanded.png")
var spr_step = 20
...
func state_wait():
	...
	if HP <= 0:
		match is_on_floor():
			...
			false: $AniPlay.play("ouchf" if ((Time.get_ticks_msec()/125) % 2 == 0) else "ouchb")
...
var HP = 100
func take_damage(damage, knockback, stun, source):
	HP -= damage
...
func tail_logic():
	if tail.size() != 0:
		$Tail.position = tail[frame]*Vector2(facing,1)
		$Tail.scale.x = facing
		if is_on_floor():
			if !hardtail:################################################## TAIL SPIN, IDLE, RUN ###
				...
				elif (abs(velocity.x) < 100) && ($Tail.animation == "flop1") or ($Tail.animation == "flop2"):
					$Tail.play("wag1")
